...
import tablebase_online as table
...
def best_move(board, timeleft) :

    book_move = book.move_from_book(board)
    if book_move != None :
        return book_move, 0
    table_move = table.tablebase(board)
    if table_move != None :
        return table_move, 0

    move, evalu = iterative_deepening(board, 4)
